    ...
    <transition name="slideleft">
        <div :class="$store.state.showHeaderMenu ? 'move-in' : 'move-out'" class="transition-all delay-500 max-w-full relative flex flex-col bg-gray-100 w-80">
            <div class="w-full flex justify-end bg-white">
                <button @click="$store.state.showHeaderMenu=false" aria-label="Search button" class="item flex justify-center items-center p-1 bg-primary rounded-md m-1 hover:bg-gray-800">
                    <si-svg><svg aria-hidden="true" focusable="false" data-prefix="fal" data-icon="times" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" class="w-5 h-5 translate"><path fill="white" d="M193.94 256L296.5 153.44l21.15-21.15c3.12-3.12 3.12-8.19 0-11.31l-22.63-22.63c-3.12-3.12-8.19-3.12-11.31 0L160 222.06 36.29 98.34c-3.12-3.12-8.19-3.12-11.31 0L2.34 120.97c-3.12 3.12-3.12 8.19 0 11.31L126.06 256 2.34 379.71c-3.12 3.12-3.12 8.19 0 11.31l22.63 22.63c3.12 3.12 8.19 3.12 11.31 0L160 289.94 262.56 392.5l21.15 21.15c3.12 3.12 8.19 3.12 11.31 0l22.63-22.63c3.12-3.12 3.12-8.19 0-11.31L193.94 256z" class=""></path></svg></si-svg>
                </button>
            </div>
            <!-- LOGO   -->
            <div class="logo flex justify-center items-center p-4">
                <router-link to="/">
            ...
            <div v-if="menu">
                <ul v-for="(item, i) in menu.items" :key="i" class="flex flex-col">
                    ...
                    <li class="flex items-center mb-1 justify-between w-full rounded-md bg-white">
                        <router-link class="p-2 m-1 w-full flex hover:text-red-700" :to="item.url">{{ item.text }}</router-link>
                    </li>
            ...
            <div>
                <ul v-for="(item, i) in otherMenu" :key="i" class="flex flex-col">
                    <li class="flex items-center mb-1 justify-between w-full rounded-md bg-white">
                        <a class="p-2 m-1 w-full flex hover:text-red-700" :href="item.url">{{ item.text }}</a>
                    ...
                    <div v-if="item._id == activeId">
                        <div v-for="(item,i) in item.childrens" :key="i" class="bg-gray-100">
                            <a class="p-2 m-1 rounded-md bg-gray-50 hover:bg-white hover:text-red-700 flex" :href="item.url">{{item.text}}</a>
                            <ul class="p-2" v-if="item.childrens && item.childrens.length > 0">
                                <li v-for="(child,ii) in item.childrens" :key="ii">
                                    <a class="p-2 m-1 rounded-md bg-gray-50 hover:bg-white hover:text-red-700 flex" :href="child.url">
                                        {{ child.text }}
                                    </a>
                                </li>
                            </ul>
            ...
            <!-- STORE NAME -->
                <div class="font-semibold text-2xl text-center my-12 mx-5 text-primary">
                    <marquee behavior="" direction="">{{ storeName }}</marquee>
                </div>
        </div>
     </transition>
</div>
...
export default {
    data() {
        return {
            ...
            storeName: this.$settings.sections.footer.copyright.store_name,
