        ...
        <div class="flex p-3 pb-0 justify-between items-center container h-14">
            <div class="flex items-center justify-between gap-6">
                ...
                <div class="logo flex justify-center items-center">
                    <router-link to="/">
                        <si-image  property="height" class="h-12 w-full w-full object-contain" :src="section.logo ? section.logo.src : $store.state.defaults.logo" alt="Store logo"/>
                    </router-link>
                </div>
            </div>
            
                <!-- HOME    FOODS    CART  AND CONTACT INFO-->
            <fragments-menu></fragments-menu>
            <div class="flex gap-3 items-center">
                <fa :icon="['fas','phone-volume']" class="text-4xl text-primary"/>  
                <!-- <img src="/images/call.png" alt="" width="50" height="50"> -->
                <div class="flex flex-col">
                    <span>{{phone.title}}</span>
                    <span><a :href='`tel:${phone.number}`' class="text-xl text-primary font-bold">{{phone.number}}</a></span>
                </div>
            </div>

            <div class="icons flex items-center justify-end">
                <button v-if="$settings.sections.header.icons.search" @click="showSearch=true" aria-label="Search button" class="item p-2 rounded-md mx-1 relative hover:shadow">
                    <si-svg><svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="search" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-6 h-6 translate"><path d="M508.5 468.9L387.1 347.5c-2.3-2.3-5.3-3.5-8.5-3.5h-13.2c31.5-36.5 50.6-84 50.6-136C416 93.1 322.9 0 208 0S0 93.1 0 208s93.1 208 208 208c52 0 99.5-19.1 136-50.6v13.2c0 3.2 1.3 6.2 3.5 8.5l121.4 121.4c4.7 4.7 12.3 4.7 17 0l22.6-22.6c4.7-4.7 4.7-12.3 0-17zM208 368c-88.4 0-160-71.6-160-160S119.6 48 208 48s160 71.6 160 160-71.6 160-160 160z" class=""></path></svg></si-svg>
                ...
                <a v-if="$settings.sections.header.icons.account" href="https://josuouedraogo.storeino.world/account/login" title="Account" class="item hidden md:block p-2  rounded-md mx-1 hover:shadow">
                    <si-svg><svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="user" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" class="w-6 h-6 translate"><path fill="currentColor" d="M313.6 304c-28.7 0-42.5 16-89.6 16-47.1 0-60.8-16-89.6-16C60.2 304 0 364.2 0 438.4V464c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48v-25.6c0-74.2-60.2-134.4-134.4-134.4zM400 464H48v-25.6c0-47.6 38.8-86.4 86.4-86.4 14.6 0 38.3 16 89.6 16 51.7 0 74.9-16 89.6-16 47.6 0 86.4 38.8 86.4 86.4V464zM224 288c79.5 0 144-64.5 144-144S303.5 0 224 0 80 64.5 80 144s64.5 144 144 144zm0-240c52.9 0 96 43.1 96 96s-43.1 96-96 96-96-43.1-96-96 43.1-96 96-96z" class=""></path></svg></si-svg>     
                </a>
                <router-link v-if="$settings.sections.header.icons.wishlist" to="/wishlist" title="Wishlist" class="item hidden md:block p-2 rounded-md mx-1 relative hover:shadow">
                    <si-svg><svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="heart" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-6 h-6 translate"><path fill="currentColor" d="M458.4 64.3C400.6 15.7 311.3 23 256 79.3 200.7 23 111.4 15.6 53.6 64.3-21.6 127.6-10.6 230.8 43 285.5l175.4 178.7c10 10.2 23.4 15.9 37.6 15.9 14.3 0 27.6-5.6 37.6-15.8L469 285.6c53.5-54.7 64.7-157.9-10.6-221.3zm-23.6 187.5L259.4 430.5c-2.4 2.4-4.4 2.4-6.8 0L77.2 251.8c-36.5-37.2-43.9-107.6 7.3-150.7 38.9-32.7 98.9-27.8 136.5 10.5l35 35.7 35-35.7c37.8-38.5 97.8-43.2 136.5-10.6 51.1 43.1 43.5 113.9 7.3 150.8z" class=""></path></svg></si-svg>
                ...
                <router-link v-if="$settings.sections.header.icons.cart" to="/cart" title="Cart" id="cart-icon" class="item p-2 rounded-md mx-1 relative hover:shadow">
...
<script>
import SocialMedia from '../fragments/SocialMedia.vue';
export default {
  components: { SocialMedia },
    data() {
        return {
            ...
            phone: this.$settings.sections.header.telephone
        }
    },
    watch: {
        showSearch(val) {
