<template>
  <div class="container">
        ...
        <div class="p-2 relative flex flex-col">
          ...
          >
            <svg
              ...
              viewBox="0 0 512 512"
              class="w-5 h-5 translate text-red-600"
            >
              <path
                fill="currentColor"
        ...
        <div class="p-2">
          <div class="bg-white p-2">
            ...
            >
              ...
              >
                <a href="#reviews"
                  >Read Reviews({{ item.review.reviews.length }})</a
                >
              </span>
            </div>
            ...
            <div class="flex justify-start items-center gap-3">
              ...
              ></si-product-quantity>
              <button
                v-if="$settings.sections.product.buy_now.active"
                v-show="
                  !$store.state.apps.find(
                    (a) => a.placement.indexOf('REPLACE_BUYNOW') >= 0
                  )
                "
                @click="buyNow"
                class="bg-primary text-white click-effect buy"
              >
                <a href="https://josuouedraogo.storeino.world/checkout2/"
                  ><span class="w-full"
                    >+ {{ $settings.sections.product.buy_now.text }}</span
                  ></a
                >
              </button>
            </div>
          ...
          <div
            class="my-2 bg-white"
            v-if="$settings.sections.product.share_buttons"
          >
            <div class="flex items-center">
              <b class="whitespace-nowrap p-2">{{
                $settings.sections.product.share_buttons.title
              }}</b>
              <div class="flex justify-center">
                <div
                  ...
                  :key="item.name"
                  class="h-12 m-2 flex items-center justify-center"
                >
                  <a
                    class="h-full flex items-center share-icons"
                    :href="item.url"
                    target="_blank"
    ...
    <div class="flex flex-col">
      ...
      <div
        class="border my-5"
        v-if="$settings.sections.product.description.active"
      >
        <h2 class="text-2xl m-4">
          {{ this.$settings.sections.product.description.title }}
        </h2>
        <div
          v-if="item"
          class="bg-white rounded-md p-2 my-3 mx-2 description"
          id="description"
          v-html="item.html"
        ></div>
      </div>
...
export default {
  ...
  async fetch() {
    ...
    try {
      ...
      this.$store.state.seo.keywords =
        this.item.seo.keywords.length > 0
          ? this.item.seo.keywords || []
          : this.$settings.store_keywords || [];
      if (this.item.images.length > 0) {
        this.$store.state.seo.image = this.item.images[0].src;
      }
      // New meta tags
      [
        {
          ...
          content: this.price.salePrice,
        },
        {
          hid: "productID",
          itemprop: "productID",
          content:
            this.product && this.product ? this.product._id : "productID",
        },
      ].forEach((meta) => {
        const index = this.$store.state.seo.metaTags.findIndex(
          (m) => m.hid === meta.hid
        );
        if (index > -1) {
          this.$store.state.seo.metaTags.splice(index, 1, meta);
        }
      ...
      if (this.item.type == "variable" && this.item.variants.length > 0)
        this.variantSelected(this.item.variants[0]);
      ...
      for (const button of this.socialMedia) {
        button.url = button.url
          .replace(/\{title\}/gi, this.item.name)
          .replace(/\{url\}/gi, url);
      }
      if (!process.server) {
  ...
  methods: {
    ...
    buyNow() {
      ...
      setTimeout(() => {
        window.location.href =
          "https://josuouedraogo.storeino.world/checkout2/";
      }, 500);
    },
    ...
    setTab(tab) {
      this.tab = tab;
      if (tab == "reviews" && this.reviews.results.length == 0)
        this.getReviews();
    },
  },
};
